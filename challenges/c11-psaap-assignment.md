...
- It looks like all the values are numbers
- idx seems like some sort of counter?
...
- Which model is *most accurate*? Which is *least accurate*?
  - Most accurate is `fit_cheat`, least accurate is `fit_nonphysical`
- What *Category* of variable is `avg_T`? Why is it such an effective
  predictor?
  - It’s category is output, which makes it an effective predictor
...
- Would we have access to `avg_T` if we were trying to predict a *new*
  ...
  - We would not becuse it’s an output value, so it’s not really a valid
    predictor
- What *Category* of variable is `idx`? Does it have any physical
  meaning?
  - idx is metadata, and it’s just telling us which run of the
    simulation it is
...
## INPUT VALUES: L, W, U_0, N_p, k_f, T_f, rho_f, mu_f, lam_f, C_fp, rho_p,
## d_p, C_pv, h, I_0, eps_p

fit_q4 <- 
  df_train %>% 
  lm(formula = T_norm ~ x + W + L + U_0 + rho_f + mu_f)

## W is decent, d_p maybe ok
...
**Observations**: - initially was trying various variables individually,
adding x to all of them as a baseline helped - some variables actually
hurt the fit rather than helped (made the rsquare on the validation data
decrease): N_p, k_f (slightly), T_f, lam_f, C_fp, rho_p, d_p (this one
was really bad on the validation), C_pv, h, I_0 (also really bad on the
validation?), eps_p (also) - Talking to other people, the above comment
is actually specific to the context in which I was looking, which was in
specific pairings with other variables rather than independently
...
- Which model tends to be more accurate? How can you tell from this
  ...
  - model q4 appears to be more accurate. The line of slope 1 will show
    us where the data “should” be if predicted = actual, so looking at
    how closely the data follows that line can show us which is more
    accurate
...
- How many predictors does the `fit_simple` model need in order to make
  ...
  - `fit_simple` uses one while my model `fit_q4` uses 6
...
``` r
sum(df_validate$T_norm > 0) # total values 
```

    ## [1] 60

``` r
sum(df_validate$T_norm > intervals_q6$pi_lwr & df_validate$T_norm < intervals_q6$pi_upr)
```
...
- How much do you trust your model? Why?
  - I think it’s a pretty decent model - 56/60 validation values fall
    within the predicted range, and it has an R^2 value of .676 which is
    not super high but still reasonable
- What kind of interval—confidence or prediction—would you use for this
  task, and why?
  - Prediction interval because in this case we’re predicting the value
- What fraction of validation cases lie within the interval you predict?
  How does this compare with `pr_level`?
  - 56 / 60 = .93, which is a bit higher than pr_level
- What interval for `T_norm` would you recommend the design team to plan
  around?
  - I would suggest they plan around the interval 1.45685 to 2.296426
- Are there any other recommendations you would provide?
  - For reference for the designers, I would also tell them our model is
    about 93% confident that the value would fall within this range
